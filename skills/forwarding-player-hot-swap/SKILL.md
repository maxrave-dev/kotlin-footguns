...
Some designs need a *new* player object per track — one media item each, two alive at once during
a transition. Everything above (the media session, the notification, the UI) wants one object that
never changes. A `ForwardingPlayer` whose delegate can be replaced bridges the two:
...
Snapshot with `toList()` before the loop — defensive, because a listener callback that re-enters
`addListener`/`removeListener` during the cycle would otherwise mutate the list mid-iteration.
...
Treat this as a version-coupled assumption: pin the library version, and re-run one swap after
every upgrade.
...
`hasNextMediaItem()` / `hasPreviousMediaItem()` are always false, and the session concludes there
is nothing to skip to: the next/previous buttons disappear from the system notification and from
external controllers. Inject a tiny provider backed by your real queue and override **both** the
per-command query and the command set:
...
`currentMediaItemIndex < timeline.windowCount` when it builds position info, and your delegate's
timeline genuinely has one window. Index and count must stay consistent with the delegate
(count 1, index 0); only the *navigation* answers come from your queue.
...
Dispatch them by hand right after the swap — item transition, metadata, and available-commands, the
last so the buttons are re-evaluated against the new queue position. Wrap each listener call in its
own `try`/`catch`: one badly behaved listener must not abort the notification for the rest.
