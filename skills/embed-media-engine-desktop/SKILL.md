...
JVM-side job is almost entirely about **ordering and thread ownership**: what must be set before
initialize, what must exist before the first load, and which thread is allowed to touch a handle.
Get those wrong and the failures are late, rare, and blame the wrong code.
...
**One thread is not enough when the engine's own event thread also writes.** The pump thread
handles events like "the audio output was re-created" by re-applying levels, so it issues property
writes too, and it cannot hop anywhere. Any routine that reads several fields and then issues more
than one native write needs a lock around the whole read-and-write, or two writers interleave and
leave the device on a combination neither of them intended. Volatile fields prevent torn reads;
they do not prevent interleaved *pairs* of calls.
...
- **Java side.** The engine's parsers accept `.` and nothing else. A default locale that formats
  with a comma is rejected silently — the setter returns a failure nobody reads, and the feature
  simply never moves. Use `String.format(Locale.ROOT, "%.4f", value)` for every number you hand
  the engine.
...
**Shut down in order, and join without a timeout.** Stop the pump and join it *before* destroying
the core: the pump may be inside a call against this handle (its own event handlers call back into
the engine), and a bounded join gives up exactly when the core is busiest tearing down. Free the
render context before destroying the core. Both steps block, so run the whole sequence off the
thread that drives playback. Have the pump loop re-check its stop flag *after* waking, so no new
native call starts once shutdown has begun, and wake it explicitly instead of waiting out its
poll timeout.
