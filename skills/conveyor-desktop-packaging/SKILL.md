...
`mac.url-schemes` / `windows.url-schemes` / `linux.url-schemes` parses fine and does nothing — the
shipped installers then register **no scheme at all**, on any platform. Nothing surfaces that — the
format is silent — so the misplacement simply persists; in the case this was mined from it sat inert
from the release that first carried the key until the key was moved. Per-OS sections do own genuinely
per-OS keys (the plist entry, the start-menu group and the desktop-file group above are all real),
which is exactly why the wrong placement looks plausible; the config's own comments are usually the
only in-repo record of which key belongs where. The way to tell them apart is not intuition: build
one machine and read the generated metadata (below).
...
(`/usr/bin:/bin:/usr/sbin:/sbin`), because the launch services layer applies the block instead of
inheriting the user's environment. Before adding one for an allocator or locale setting, audit every
place the app spawns a subprocess; anything resolved by bare name outside those four directories
stops being found, and only when launched from the desktop — running the binary from a shell still
works, which makes it look intermittent. The same block is also not applied at all for a direct
shell launch, so a setting you rely on is absent in exactly the configuration you debug in.
